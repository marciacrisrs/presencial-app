...
class OnboardingEligibilityTest {
    ...
    fun `nextStep normaliza extremos`() {
        assertEquals(
            ...
            OnboardingEligibility.nextStep(-10)
        )
        assertEquals(
            OnboardingEligibility.STEP_LOCATION,
            OnboardingEligibility.nextStep(Int.MAX_VALUE)
        )
        assertEquals(
            OnboardingEligibility.STEP_LOCATION,
            OnboardingEligibility.nextStep(OnboardingEligibility.STEP_LOCATION)
        )
        assertEquals(
            OnboardingEligibility.STEP_REMINDER,
